		...
		<div class="reaize_box" ref="box" id="reaize_box">
			<div class="left" ref="left" id="left">
				<div class="leftTreeWrap box treeBox">
					<vehicleTree deviceType="2" @ready="ready"></vehicleTree>
				</div>
			</div>
			...
			<div class="right table-wrapper" ref="right" id="right">				
		          <el-form :inline="true" class="demo-form-inline" size="small">
		              <el-form-item label="MN号查询">
		                <el-input placeholder="请输入内容" v-model="formquery.mnNum"></el-input>
		              </el-form-item>
					...
					<avue-crud :option="option" :table-loading="loading" :data="data" :page="page" :permission="permissionList" :before-open="beforeOpen" :before-close="beforeClose" v-model="form" ref="crud" @row-update="rowUpdate" @row-save="rowSave" @row-del="rowDel" @search-change="searchChange" @search-reset="searchReset" @selection-change="selectionChange" @current-change="currentChange" @size-change="sizeChange" @refresh-change="refreshChange" @on-load="onLoad">
						...
						<template slot="menuLeft">
							<el-button type="primary" size="small" icon="el-icon-plus" plain v-if="permission.deviceinfo_addrdtu" @click="handleAddrdtu">新增数采仪
							</el-button>
							<el-button type="primary" size="small" icon="el-icon-plus" plain v-if="permission.deviceinfo_addinst" @click="handleAddinst">新增设备仪器
							</el-button>
							<el-button type="primary" size="small" icon="el-icon-plus" plain v-if="permission.deviceinfo_addinst" @click="addFactors">批量添加因子
							</el-button>
							<el-button type="danger" size="small" icon="el-icon-delete" plain v-if="permission.deviceinfo_delete" @click="handleDelete">删 除
							</el-button>
						</template>
						<!-- 自定义表格行内编辑按钮 -->
						<template slot="menu" slot-scope="scope">
							<el-button type="text" icon="el-icon-edit" size="small" v-if="permission.deviceinfo_edit" @click="handleEdit(scope.row)">编辑
							</el-button>
							<el-button type="text" icon="el-icon-delete" size="small" v-if="permission.deviceinfo_delete" @click="handleDel(scope.row)">删除
							</el-button>
						</template>
					</avue-crud>
					<chooseFactor
...
<script>
	import { getListByVehicle, getDetail, add, update, remove, getDeptTree, checkCountByParentId, getVehicleListByMn } from "@/api/ficm/deviceinfo";
	...
	export default {
        ...
        // 后台接口变化 增加判断treeType是2 则是站点
			...
			//新增数采仪
			handleAddrdtu() {
				if(this.isSiteInfo) { //nodeId修改成isSiteInfo 判断是否选择了站点
					this.$router.push({
						path: '/rdtuinfo',
						query: {
							deptInfoId: this.deptInfoId, ////2020/10/12修改树到站点 企业id
      ...
      searchBtn(){
          ...
          this.loading = true;
          getVehicleListByMn(this.page.currentPage, this.page.pageSize, this.formquery.mnNum).then(res=>{
        ...
        this.formquery = {};
				if(this.nodeId) {
					this.loading = true;
					getListByVehicle(page.currentPage, page.pageSize, this.nodeId, Object.assign(params, this.query)).then(res => {
						const data = res.data.data;
						this.page.total = data.total;
						this.data = data.records;
						this.loading = false;
