		...
		<div class="reaize_box" ref="box" id="reaize_box">
			...
			<div class="right table-wrapper" ref="right" id="right">
				<avue-crud :option="option" :table-loading="loading" :data="data" :page="page" :permission="permissionList" :before-open="beforeOpen" 
					:before-close="beforeClose" v-model="form" ref="crud" @row-update="rowUpdate" @row-save="rowSave" @row-del="rowDel" 
					...
					<template slot="menuLeft">
						<el-button type="danger" size="small" icon="el-icon-delete" plain v-if="permission.locationinfo_delete" @click="handleDelete">删 除
						</el-button>
					</template>
				</avue-crud>				
			</div>
...
<script>
	...
	export default {
		...
		computed: {
			...
			permissionList() {
				return {
					addBtn: this.vaildData(this.permission.locationinfo_add, false),
					viewBtn: this.vaildData(this.permission.locationinfo_view, false),
					delBtn: this.vaildData(this.permission.locationinfo_delete, false),
					editBtn: this.vaildData(this.permission.locationinfo_edit, false)
				};
			},
			ids() {
				let ids = [];
		...
		watch: {
			...
			'form.operaDeptId' (val) {
				if(val) {
					...
					let userArray = column1.dicData.filter(item => item.deptInfoId == val)
					const column2 = this.findObject(this.option.group, "operaUserId")
					if(userArray && userArray.length) column2.dicData = userArray[0].userList
				}
			},
