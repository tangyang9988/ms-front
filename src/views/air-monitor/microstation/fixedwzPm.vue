...
<script>
import {getFixedSiteFactors,getAllFixedSiteRankList } from "@/api/microstation/index.js";
import { mapGetters } from "vuex";
import { getToken } from "@/util/auth";
export default {
  data() {
    return {
      message:"固定微站自排名：采用平台内的固定微站数据作为排序依据取污染程度较高的展示",
      bzAroundArea:1500,
      desc:1, //默认降序
      timeType:2,
      dateTime:new Date(),
  ...
  methods: {
    ...
    exportRoad() {
      this.$confirm("是否导出表格?", "提示", {
        ...
        window.open(`/api/ms-airq/fixedsite/exportAllFixedSiteRank?${this.website.tokenHeader}=${getToken()}&desc=${this.desc}&type=${this.timeType}&time=${this.timeTypeChange(this.dateTime)}&factorCode=${this.moveFacType}`);
      });
    },
  },
};
