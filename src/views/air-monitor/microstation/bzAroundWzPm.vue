...
<script>
import { getFixedSiteFactors,getMobilesiteFactors,getRoadRank,getMobileSiteRank,getAllStatAroundSiteRankList,getStationFactors } from "@/api/microstation/index.js";
import { mapGetters } from "vuex";
import { getToken } from "@/util/auth";
export default {
  data() {
    return {
      message:"标站排名-固定微站：以国控站点为排序内容，将平台内的固定微站数据作为排序依据，进行污染程度的排序展示；",
      bzAroundArea:1500,
      desc:1, //默认降序
      timeType:2,
      dateTime:new Date(),
  ...
  methods: {
    ...
    exportRoad() {
      this.$confirm("是否导出表格?", "提示", {
        ...
        window.open(`/api/ms-airq/fixedsite/export?${this.website.tokenHeader}=${getToken()}&desc=${this.desc}&type=${this.timeType}&time=${this.timeTypeChange(this.dateTime)}&factorCode=${this.moveFacType}&distance=${this.bzAroundArea*0.001}`);
      });
    },
  },
};
