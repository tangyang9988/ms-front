...
<script>
import { getFixedSiteFactors,getMobilesiteFactors,getRoadRank } from "@/api/microstation/index.js";
...
export default {
  ...
  created(){
     getMobilesiteFactors().then(res=>{
        ...
        this.moveFacType = res.data.data[0].id;
        console.log(this.moveFacList);
    });
  },
  mounted(){
  ...
  methods: {
    ...
    exportRoad() {
      this.$confirm("是否导出表格?", "提示", {
        ...
        window.open(`/api/ms-airq/mobilesite/exportRoadRank?${this.website.tokenHeader}=${getToken()}&desc=${this.desc}&type=${this.timeType}&time=${this.timeTypeChange(this.dateTime)}&factorCode=${this.moveFacType}`);
      });
    },
  },
};
