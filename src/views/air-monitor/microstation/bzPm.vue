...
<script>
import { getFixedSiteFactors,getStationFactors,getRoadRank,getMobileSiteRank,getAllStatAroundSiteRankList,getPollutionRank } from "@/api/microstation/index.js";
import { mapGetters } from "vuex";
import { getToken } from "@/util/auth";
export default {
  data() {
    return {
      message:"标站自排名：使用国控站点数据作为排序依据",
      bzAroundArea:1500,
      desc:1, //默认降序
      timeType:2,
      dateTime:new Date(),
  ...
  methods: {
    ...
    exportRoad() {
      this.$confirm("是否导出表格?", "提示", {
        ...
        window.open(`/api/ms-airq/stat/export?${this.website.tokenHeader}=${getToken()}&desc=${this.desc}&type=${this.timeType}&time=${this.timeTypeChange(this.dateTime)}&factorCode=${this.moveFacType}`);
      });
    },
  },
};
