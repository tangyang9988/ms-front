<template>
	<el-row class="GIS-container" :class="{GISContainerBig:mapType==1}" v-loading="loading">
		<baidu-map ref="map" class="map" :center="center" :zoom="zoom" @zoomend="zoomEnd" 
			:scroll-wheel-zoom="true" :map-click="false" @moveend="moveEnd" :mapStyle="mapStyle" @ready="handler">
			<div v-for="(item, index) in pointList">
                <bm-marker 
	            	v-if="item.factorList&&item.wqiDayLevel==1"
	                :key="index"
	                :icon="{url: '/img/bg/tuding/icon1.png', size: {width: 30, height: 30}}"
	                :position="item.position" 
	                @mouseover="pointClick(item)"	               
	            ></bm-marker>	            
                <bm-marker 
	            	v-if="item.factorList&&item.wqiDayLevel==2"
	                :key="index"
	                :icon="{url: '/img/bg/tuding/icon2.png', size: {width: 30, height: 30}}"
	                :position="item.position" 
	                @mouseover="pointClick(item)"	               
	            ></bm-marker>	
                <bm-marker 
	            	v-if="item.factorList&&item.wqiDayLevel==3"
	                :key="index"
	                :icon="{url: '/img/bg/tuding/icon3.png', size: {width: 30, height: 30}}"
	                :position="item.position" 
	                @mouseover="pointClick(item)"	               
	            ></bm-marker>	
                <bm-marker 
	            	v-if="item.factorList&&item.wqiDayLevel==4"
	                :key="index"
	                :icon="{url: '/img/bg/tuding/icon4.png', size: {width: 30, height: 30}}"
	                :position="item.position" 
	                @mouseover="pointClick(item)"	               
	            ></bm-marker>		            
                <bm-marker 
	            	v-if="item.factorList&&item.wqiDayLevel==5"
	                :key="index"
	                :icon="{url: '/img/bg/tuding/icon5.png', size: {width: 30, height: 30}}"
	                :position="item.position" 
	                @mouseover="pointClick(item)"	               
	            ></bm-marker>	
                <bm-marker 
	            	v-if="item.factorList&&item.wqiDayLevel==6"
	                :key="index"
	                :icon="{url: '/img/bg/tuding/icon6.png', size: {width: 30, height: 30}}"
	                :position="item.position" 
	                @mouseover="pointClick(item)"	               
	            ></bm-marker>					
				<!--<bm-marker v-if="formatFactorList(item.factorList)&&item.factorList" :key="index"
					 :icon="{url: '/img/bg/tuding3.png', size: {width: 30, height: 30}}" 
					 :position="item.position" @mouseover="pointClick(item)"></bm-marker>
				<bm-marker v-else-if="!formatFactorList(item.factorList)&&item.factorList" :key="index" 
					:icon="{url: '/img/bg/tuding/icon6.png', size: {width: 30, height: 30}}" :position="item.position" 
					@mouseover="pointClick(item)"></bm-marker>-->
			</div>
			<bm-info-window :position="infoData.position" :title="infoData.siteName+'-'+infoData.deviceName" :show="infoShow" :offset="{width: 0, height: -20}" :closeOnClick="false" @clickclose="withInfoClose">
				<!-- 设备状态 -->
				<div class="info_item">
		...
		<!-- 视频弹框 -->
		<el-dialog :visible.sync="videoVisible" :modal="false" :title="dialogTitle" width="65%">
...
<script>
	import { getGISData ,getSiteWqi} from "@/api/swaq/gis";
	import { mapStyle } from "./option/system.js";
