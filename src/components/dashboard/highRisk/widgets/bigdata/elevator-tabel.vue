<template>
	<div class="wrapper indexwrapper">
		<div class="header">升降机监测<span class="subTitle" @click="linkTab">历史数据</span></div>
		<div style="padding:10px;background: #1e2539;flex: 1;overflow-y: hidden;">
			<el-table :data="tableData">
				...
				<el-table-column prop="height" label="实时高度" align="center">
					<template slot-scope="scope">
						<div v-if="scope.row.height">{{scope.row.height}}</div>
						<div v-else>-</div>
					</template>						
				</el-table-column>
				<el-table-column prop="speed" label="实时速度" align="center">
					<template slot-scope="scope">
						<div v-if="scope.row.speed">{{scope.row.speed}}</div>
						<div v-else>-</div>
					</template>					
				</el-table-column>
...
<script>
	import { deviceelevatordata } from "@/api/dust/supervise";
	...
	export default {
		...
		methods: {
			linkTab() {
				this.$store.commit('SET_ELEVATOR')
				this.$router.push({
					path: '/dust/highRisk/elevator/elevator'
				})
			},			
			formatDatetime(str) {
				if(str) {
