<template>
	<div>
		...
		<grid-layout :layout.sync="layout" :col-num="12" :row-height="rowHeight" :max-rows="maxRows" :is-draggable="false" :is-resizable="false">
			<grid-item class="panel" v-for="item in layout" :x="item.x" :y="item.y" :w="item.w" :h="item.h" :i="item.i" :key="item.i" @mouseover.native="onEnterPanel(item.i)" @mouseleave.native="onLeavePanel(item.i)">
				<!-- 浮动按钮 -->
				<el-button v-show="showPanelButtons(item.i)" class="panel_button" @click="delWidget(item.i)" size="mini" type="danger" icon="el-icon-delete" circle></el-button>
				<!-- 动态widget -->
			...
			init() {
				getDeptCategory().then(res => {
					...
					opts().then(res => {
						...
						getDetail(that.userInfo.tenant_id, localStorage.getItem('systemType'), that.deptType, '5').then(res => {
