    ...
    >
      ...
      >
        <template slot-scope="scope">
          ...
          <!--AQI-->
          <div v-else-if="item.prop=='aqiValAll'">{{ scope.row[item.prop].avgVal }}</div>
          <!--因子显示内容-->
          <div v-else>
            <!--文字提示-->
            <el-tooltip placement="top">
              <div slot="content">
                <p>iaqi值: {{ scope.row[item.prop].aqiVal }}</p>
              </div>
              <div> {{ scope.row[item.prop].avgVal || '-' }}</div>
            </el-tooltip>
          </div>
          <!--右侧图表按钮-->
          <div class="icon-right" :class="getRowLevelColor(scope.row[item.prop])"
          	v-if="(item.prop=='aqiValAll'&& scope.row[item.prop].avgVal)||(isFactor(item.prop)&&scope.row.aqiLevel&&scope.row[item.prop].aqiVal)">
          </div>
        </template>
      </el-table-column>
    </el-table>
...
<script>
import {getTableData} from "@/api/airq/airqIndex";
