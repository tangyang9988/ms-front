<template>
  <div class="chart_wrapper">
    <div class="header">本月治理评级</div>
    <e-chart autoresize :options="chartOptions" style="height:100%;"></e-chart>
  </div>
</template>

<script>
import { getGovernanceRating } from "@/api/loam/big-data";
...
export default {
  ...
  mounted() {
    getGovernanceRating().then(res => {
      // test only
      // res.data.data = { level1: 11, level2: 36, level3: 8, level4: 2, totalNumberOfCompanies: 57 };
      ...
      var totalNum = res.data.data.totalNumberOfCompanies
        ...
        : 0;
      this.chartOptions.series[0].label.normal.formatter = "参评企业总数\n\n" + totalNum;
    });
  }
};
</script>
