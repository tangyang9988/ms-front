...
<script>
import {getdDustfallData} from "@/api/dust/eveindex";
import $ from "jquery"
  ...
  methods: {
    getTableData() {
      getdDustfallData().then(res => {
        ...
        // 遍历数组
        this.tableData = []//清空写死数据
        // 遍历数组 显示前6条数据
        const resData = []
        // console.log('降尘控制', resData)
        ...
        resData.forEach(item => {
          this.tableData.push({
            name: item.name,
            contrType: this.contrTypeTransform(item.contrType),
            tspVal: item.tspVal,
            sprayingDuration: item.sprayingDuration,
          })
        })
      ...
      // if (this.tableData.length <= 0) {//如果请求接口错误 显示写死数据
      //   this.tableHead = [
      //     {
      //       label: '设备名称',
      //       prop: 'name',
      ...
      //   ]
      //   this.tableData = [
      //     {'name': 'xxxxxx', 'contrType': '自动', 'tspVal': '0', 'sprayingDuration': '10'},
      //     {'name': 'xxxxxx', 'contrType': '自动', 'tspVal': '0', 'sprayingDuration': '10'},
      //   ]
      // }
    },
    contrTypeTransform(val) {
      if (val == 2) {
        return val = '自动'
      } else {
        return val = '手动'
