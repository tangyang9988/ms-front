<template>
	<el-row class="GIS-container" v-loading="loading">
		<baidu-map ref="map" class="map" :center="center" :zoom="zoom" 
			@zoomend="zoomEnd" :scroll-wheel-zoom="true" 
			:map-click="false" @moveend="moveEnd" @ready="handler">
			<div v-for="(item, index) in pointList">
                ...
                <bm-marker 
	                ...
	                @mouseover="pointClick(item)"	               
	            ></bm-marker>					
				<!--<bm-marker v-if="formatFactorList(item.factorList)&&item.factorList" :key="index" :icon="{url: '/img/bg/tuding3.png', size: {width: 30, height: 30}}" :position="item.position" @mouseover="pointClick(item)"></bm-marker>-->
				<!--<bm-marker v-else-if="!formatFactorList(item.factorList)&&item.factorList" :key="index" :icon="{url: '/img/bg/tuding/icon6.png', size: {width: 30, height: 30}}" :position="item.position" @mouseover="pointClick(item)"></bm-marker>-->
			</div>
