...
import {loginByUsername, loginBySocial, getUserInfo, logout, refreshToken, getButtons,getSystemName} from '@/api/user'
...
const user = {
  ...
  actions: {
  	changeSystemToken({commit}){
  		let userInfo=localStorage.getItem('saber-userInfo')
  		let token=JSON.parse(userInfo).content.access_token
    ...
    LoginByUsername({commit}, userInfo) {
      return new Promise((resolve, reject) => {
        loginByUsername(userInfo.tenantId, userInfo.username, md5(userInfo.password), userInfo.type, userInfo.key, userInfo.code).then(res => {       	
          ...
          resolve();
//        getSystemName().then(resource =>{          	
//          if(resource.data.msg!='暂无承载数据'){
//            localStorage.setItem('systemName', resource.data.data)
//          }else{
...
//        })
    ...
    GetMenu({commit, dispatch}, topMenuId) {      
      return new Promise(resolve => {
        getRoutes(localStorage.getItem('systemType')).then((res) => {
          const data = res.data.data
  ...
  mutations: {
    ...
    SET_USER_INFO: (state, userInfo) => {
      if (validatenull(userInfo.avatar)) {
        userInfo.avatar = "/img/bg/huanbao.png";
      }
